...
| **Enforced now** | The 70% soft-warning ratio is computed (it is the architecture's number) but **consumed nowhere** — it reaches no operator, log line or response. Surfacing it is outstanding engineering work, not a decision. The cap is **not defaulted**: an unset cap resolves to `UNCONFIGURED` and refuses every AI feature with `BUDGET_UNCONFIGURED`. A test asserts the guardrails module supplies no cap of its own (`tests/ai-guardrails.test.ts`, which scans that one file). |
