...
export const metadata = { title: 'Check-ins — Srivillu Home Stays' };
...
export default async function CheckinsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <ReadOnlyPage
      ...
      title="Check-ins"
      description="Today's arrivals, ready to check in as guests reach the door."
      searchParams={params}
      filters={['property']}
      fetcher={(provider, f) => provider.getOperations(f)}
    >
      {(board) => <ArrivalsTable rows={board.arrivals} mode="checkin" />}
    </ReadOnlyPage>
  );
}
